...
齿轮面板支持中文、English、日本語下拉选择，界面、提示、规则说明同步切换并保存在独立偏好存储。自动过滤候选默认开启；关闭后可填入 1–9。盘面候选默认关闭，仅在自动过滤开启时可用，按 3×3 展示。笔记模式点击候选会加删除线，再点恢复，不再高亮；划去标记支持撤销和存档。关闭盘面候选时，当前笔记格及已有笔记仍显示候选。新数独的问号按住显示，释放、取消、滚动或切后台后关闭。
