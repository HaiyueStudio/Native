...
2. 已配置 `App_Resources/iOS/Info.plist` 的 `GADApplicationIdentifier`：`ca-app-pub-2053256758816744~9316994454`。
3. 已配置 `src/rewards-config.ts` 的 `iosUnit`：`ca-app-pub-2053256758816744/9110868483`。Release 构建只校验目标平台的正式 App ID、广告单元 ID 及发布商一致性，运行时也禁止请求测试广告。开发版仍强制使用 Google 测试广告位，Android 暂保留测试配置。
...
配置正式 ID 不代表已通过 AdMob 账号/应用审核，也不代表隐私消息已发布。广告库存/网络/同意状态都可能导致暂无广告，游戏会保留拼图且不扣额度。

### iOS 后台填写内容（2026-09-22）

用户已创建账号、iOS 应用和激励广告位，并提供上述正式 ID。网站授权发布商为 `pub-2053256758816744`，根路径文件为 `https://haiyuestudio.github.io/app-ads.txt`。用户已通过截图确认欧洲法规消息发布（英语及另外 5 种语言）。正式隐私政策已部署。后台付款设置、应用验证与审核、美国适用地区消息仍待确认；UMP 真机验证见 evidence/20260922-admob-consent。
...
正式 App ID 与广告位 ID 已接入；欧洲消息已发布，继续用测试设备验证同意、拒绝、重新管理选项、取消广告、奖励一次性发放、无填充和断网。
...
iOS 每次创建游戏宿主后更新 UMP；免费用户显示 SDK 要求的同意表单，已购买用户仅更新隐私状态，不预加载广告。网络失败后解除暂停，保留额度并允许正常游戏；下次显式请求广告或隐私选项时重试。设置中始终提供公开隐私政策网址，六种语言均有按钮；广告隐私选项只在 UMP 要求时显示。
