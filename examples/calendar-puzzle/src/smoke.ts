...
export async function seedCalendarSmoke(backend: GameSaveBackend): Promise<void> {
  ...
  await save.saveNow({ year:2024, month:2, day:28, weekday:3, language:'zh', completedDates:[], pieces:solution.map((p,i)=>({ ...p, x:0,y:0,layer:i,scale:1,placed:i!==9 })) });
}
export function installCalendarSmoke(engine: HaiyueEngine, game: CalendarPuzzleGame, input: NativeTouchInput,
  backend: GameSaveBackend, report: (event: string, data: unknown) => void, canvas: Canvas): () => void {
  ...
  const tick=()=>{
    ...
    if(frame===100){hintStarted=performance.now();tap('hint');}
    if(frame===106){check('native worker finds a compatible final-piece hint',s.hint?.piece===9&&s.hintCompatible&&!s.hintBusy);capture('hint-target');}
