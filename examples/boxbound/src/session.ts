...
export class MobileSession {
  ...
  private pending:Action|'hop'|null=null;private pendingUndo=false;private completion:number|null=null;private lastStep=0;
  ...
  cancel(){this.held=null;this.pending=null;this.pendingUndo=false;}
  setDirection(dir:Vec|null){this.held=dir;}
  act(action:Action){
    ...
    const before=this.state,result=advanceGame(before,action);this.state=result.state;
    if(result.changed){remember(this.history,before,action.type==='move'&&!!action.jump,result.transfers,false,result.playerCrossing);this.audio(soundCues(before,this.state,action,result.recoil,this.scene.airborne,result.playerCrossing));this.save(this.state);
  ...
  jump(){if(this.home||this.paused||this.scene.airborne||this.scene.celebrating||this.scene.transitioning||!canJump(this.state))return;
    ...
    this.scene.jumpInPlace();this.audio([{name:'jump',delay:0}]);if(this.held)this.act({type:'move',dir:[...this.held],jump:true});
  }
