...
const resolved = path.join(app, 'platforms/ios/skystrike.xcworkspace/xcshareddata/swiftpm/Package.resolved');
mkdirSync(path.dirname(resolved), { recursive: true });
copyFileSync(path.join(app, 'locks/Package.resolved'), resolved);
run('bundle', ['exec', 'xcrun', 'xcodebuild',
  '-workspace', 'platforms/ios/skystrike.xcworkspace', '-scheme', 'skystrike',
