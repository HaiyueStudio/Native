...
import json,struct,hashlib,shutil,os
root=Path(__file__).resolve().parents[3]
source=root/'games/neon-circuit/assets'
model_source=Path(os.environ.get('HAIYUE_MODEL_ASSETS', str(root/'local-assets')))/'neon-circuit/wraith-raider.glb'
if not model_source.is_file():
 raise SystemExit(f'Missing user-provided model: {model_source}. See games/ASSETS.md.')
out=root/'examples/neon-circuit/src/game-assets'
...
glb=model_source.read_bytes();offset=12;gltf=None;binary=None
