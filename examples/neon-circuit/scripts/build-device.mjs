...
if (jsOnly && !existsSync(path.join(app, 'platforms/ios/neoncircuit.xcworkspace'))) throw new Error('Run a full device build before using --js-only.');
run(process.execPath, ['scripts/ios.mjs', 'prepare', 'ios', ...(jsOnly ? ['--skipNative'] : [])]);
const resolved = path.join(app, 'platforms/ios/neoncircuit.xcworkspace/xcshareddata/swiftpm/Package.resolved');
mkdirSync(path.dirname(resolved), { recursive: true });
copyFileSync(path.join(app, 'locks/Package.resolved'), resolved);
run('bundle', ['exec', 'xcrun', 'xcodebuild',
  '-workspace', 'platforms/ios/neoncircuit.xcworkspace', '-scheme', 'neoncircuit',
